...
function job_filters_leads()
{
    ...
    ##########################################
	// Agentes Telefonicos
    $sqlQuery2 = "insert IGNORE into unifin.filters(id, name, date_entered, date_modified, modified_user_id, created_by, description, deleted, team_id, team_set_id, assigned_user_id,filter_definition,filter_template,module_name, acl_team_set_id)
select u.id,
       'Mis Leads' as name,
...
from unifin.users u, unifin.users_cstm uc
where u.id= uc.id_c and uc.puestousuario_c = '27'
and u.status = 'Active';
    ";
    $result2 = $GLOBALS['db']->query($sqlQuery2);
	// Asesores
    $sqlQuery3 = "insert IGNORE into unifin.filters(id, name, date_entered, date_modified, modified_user_id, created_by, description, deleted, team_id, team_set_id, assigned_user_id,filter_definition,filter_template,module_name, acl_team_set_id)
select uuid(),
       'Mis Leads' as name,
       '2021-02-26 18:00:00' as date_entered,
       '2021-02-26 18:00:00' as date_modified,
       ...
       u.team_set_id as team_set_id,
       null assigned_user_id,
       concat('[{\"assigned_user_id\":{\"\$in\":[\"',u.id,'\"]}},{\"contacto_asociado_c\":{\"\$equals\":\"1\"}}]') as filter_definition,
       concat('[{\"assigned_user_id\":{\"\$in\":[\"',u.id,'\"]}},{\"contacto_asociado_c\":{\"\$equals\":\"1\"}}]') as filter_template,
       'Leads' as module_name,
       null as acl_team_set_id
from unifin.users u, unifin.users_cstm uc
