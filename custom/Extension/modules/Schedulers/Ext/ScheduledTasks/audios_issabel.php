...
array_push($job_strings, 'audios_issabel');
/**
...
*/
   ...
   function audios_issabel(){
        ...
        * 1.-Conectar al FTP y validar que haya archivos nuevos
        */
        $GLOBALS['log']->fatal('INICIA JOB ISSABEL, CONEXION A FTP AUDIOS');

        set_include_path(get_include_path() . PATH_SEPARATOR . 'custom/aux_libraries/phpseclib1.0.18');
        require_once('Net/SFTP.php');
        // Asignacion de Variables 
        $host = $sugar_config['issabel_sftp_host'];
        $port = $sugar_config['issabel_sftp_port'];
        $username = $sugar_config['issabel_sftp_usr'];
        $password = $sugar_config['issabel_sftp_pass'];
        $route= $sugar_config['issabel_sftp_route'];
            ...
            if (!empty($directorio)){
                ...
                foreach($directorio as $file){
                    if (strlen($file)>2) {
                        ...
                        $exe_upload=upload_audios1($exe_variables,$file_name,$contentfile,$consult_idM,$iduser);
                    }
                }
            }
        }else{
    ...
    function get_one_drive_config1()
    {
    ...
    function upload_audios1($exe_variables, $file_name, $file_content=null,$consult_idM,$iduser)
    {
    ...
    function consultDrive1($exe_variables,$parentFolder,$nameFolder,$iduser)
    {
        //Arma petición para token. Peticion AÑO
        //Valor folder es de TEST= 01GKB4L5AMUFV3EFVNOFHIS43SYU5WIQEW
    ...
    function createfolder1($exe_variables,$iduser,$parentFolder,$nameFolder){
        $GLOBALS['log']->fatal("Inicia createfolder1");
        $uri = 'https://graph.microsoft.com/v1.0/users/'.$iduser.'/drive/items/'.$parentFolder.'/children';
        ...
        curl_close ($ch);
        $GLOBALS['log']->fatal("Sale createfolder1");
