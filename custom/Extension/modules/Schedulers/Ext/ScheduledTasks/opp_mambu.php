...
function opp_mambu()
{
    global $sugar_config;
//Inicia ejecución
    $GLOBALS['log']->fatal('Job para mandar Solicitudes UNICLICK a Mambu: START');
...
WHERE ac.tipo_registro_c='Cliente'campo
...
and oc.tct_id_mambu_c is null";
//Ejecuta Consulta
    $resultOpp = $GLOBALS['db']->query($sqlQueryOpp);
    while ($row = $GLOBALS['db']->fetchByAssoc($resultOpp)) {
        $GLOBALS['log']->fatal('Solicitudes a crear en Mambu: ' . count($resultOpp));
        $contador = 0;
        ...
        $GLOBALS['log']->fatal('Respuesta Solicitud a Mambu: ' .json_encode($resultado));
        ...
        if (!empty($resultado['encodedKey'])) {
            ...
            $GLOBALS['log']->fatal("Esta Solicitud ya existe en Mambu");

        }
    }
    $GLOBALS['log']->fatal('Se han creado ' . $contador . ' cuentas');
    $GLOBALS['log']->fatal('Job para mandar Solicitudes UNICLICK a Mambu: END');
