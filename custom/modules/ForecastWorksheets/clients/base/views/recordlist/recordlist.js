...
({
    ...
    initialize: function (options) {
        ...
        this.context.set('skipFetch', !(this.selectedUser.showOpps || !this.selectedUser.is_manager));
        ...
        if (!_.isUndefined(this.context.parent) && !_.isUndefined(this.context.parent.get('model'))) {
            if (this.context.parent.get('model').module == 'Forecasts') {
                ...
                this.context.parent.on('button:commit_button:click', function () {
                    if (this.layout.isVisible()) {
                        /** BEGIN CUSTOMIZATION: jgarcia@levementum.com 9/1/2015 Description: if an Operación is marked as Excluido the system will request a new “Fecha de Cierre Esperada”.*/
                        this.collection.each(function (model) {
                            if(model.get('commit_stage') == 'exclude') {
                                this.stage = model.get('commit_stage');
                                var date = model.get('date_closed');
                                this.diffM = this.diffMonths(date);
                            }
                        },this);
                        if(this.diffM >= 0 && this.stage == 'exclude'){
                            app.alert.show("excludeOpp", {
                                level: "error",
                                title: "Porfavor, mueva la fecha de cierre de este mes",
                                autoClose: false
                            });
                        }
                        else {
                ...
                this.collection.on('change:commit_stage', function (model) {
                    if (!_.isEmpty(this.filters) && _.indexOf(this.filters, model.get('commit_stage')) === -1) {
                        this.filterCollection();
                        if (!this.disposed) {
                            this.render();
                        }
                        ...
                        /* END CUSTOMIZATION */
                        var commitStage = model.get('commit_stage'), includedCommitStages = app.metadata.getModule('Forecasts', 'config').commit_stages_included, el = this.$el.find('tr[name=' + model.module + '_' + model.id + ']'), isIncluded = _.include(includedCommitStages, commitStage);
                        if (el) {
    ...
    beforeRenderCallback: function () {
        var showOpps = (_.isUndefined(this.selectedUser.showOpps)) ? false : this.selectedUser.showOpps, isManager = (_.isUndefined(this.selectedUser.is_manager)) ? true : this.selectedUser.is_manager;
    ...
    checkForDraftRows: function (lastCommitDate) {
        if (this.layout.isVisible() && this.canEdit && this.hasCheckedForDraftRecords === false && !_.isEmpty(this.collection.models) && this.isCollectionSyncing === false) {
    ...
    saveWorksheet: function (isDraft) {
        var totalToSave = 0;
        if (this.layout.isVisible()) {
            var saveCount = 0, ctx = this.context.parent || this.context;
            if (this.isDirty()) {
    ...
    calculateTotals: function () {
        if (this.layout.isVisible()) {
            ...
            var calcFields = ['worst_case', 'best_case', 'likely_case'], fields = _.filter(this._fields.visible, function (field) {
    ...
    displayLoadingMessage: function () {
        app.alert.show('workshet_loading', {level: 'process', title: app.lang.getAppString('LBL_LOADING')});
        this.collection.once('reset', function () {
    ...
    getCommitTotals: function () {
        var includedAmount = 0, includedBest = 0, includedWorst = 0, filteredAmount = 0, filteredBest = 0, filteredWorst = 0, filteredCount = 0, overallAmount = 0, overallBest = 0, overallWorst = 0, includedCount = 0, lostCount = 0, lostAmount = 0, lostBest = 0, lostWorst = 0, wonCount = 0, wonAmount = 0, wonBest = 0, wonWorst = 0, includedClosedCount = 0, includedClosedAmount = 0, cfg = app.metadata.getModule('Forecasts', 'config'), startEndDates = this.context.get('selectedTimePeriodStartEnd') || this.context.parent.get('selectedTimePeriodStartEnd'), activeFilters = this.context.get('selectedRanges') || this.context.parent.get('selectedRanges') || [];
        var sales_stage_won_setting = cfg.sales_stage_won || [], sales_stage_lost_setting = cfg.sales_stage_lost || [];
    ...
    /** BEGIN CUSTOMIZATION: jgarcia@levementum.com 9/1/2015 Description: if an Operación is marked as Excluido the system will request a new “Fecha de Cierre Esperada”.*/
    diffMonths: function(fecha_cierre) {
        var today = new Date();
        var close = new Date(fecha_cierre);
        var m = today.getMonth() - close.getMonth();
        return m;
    },
    /* END CUSTOMIZATION */
})
