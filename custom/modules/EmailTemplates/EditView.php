...
if ($has_campaign || $inboundEmail) {
    if (SugarAutoloader::fileExists('custom/modules/EmailTemplates/EditView.html')) {
        $xtpl = new XTemplate('custom/modules/EmailTemplates/EditView.html');
    } else {
        $xtpl = new XTemplate('modules/EmailTemplates/EditView.html');
    }
} else {
    if (SugarAutoloader::fileExists('custom/modules/EmailTemplates/EditViewMain.html')) {
...
if (!SugarAutoloader::fileExists(sugar_cached('jsLanguage/') . $GLOBALS['current_language'] . '.js')) {
...
if (true) {
    ...
    $query->select(array("id", "name", "survey_status"));
  ...
  //  $runQuery = $db->query($selectSurvey);
    ...
    if (isset($_REQUEST['survey_id'])) {
        ...
        $selected_survey_name = $oSurvey->name;
    }
    ...
    foreach ($runQuery as $surveyresult) {
        ...
        if (!empty($selected_survey) && $selected_survey == $surveyresult['id']) {
            $sel = "selected=selected";
            $surveyDDOption .= "<option value='{$selected_survey}' $sel>{$selected_survey_name}</option>";
        } else {
            $surveyDDOption .= "<option value='{$surveyresult['id']}' $sel>{$surveyresult['name']}</option>";
        }
    }
    ...
    $sugar_survey_Url = $sugar_config['site_url'] . $survey_url_for_email;
    $xtpl->assign("SURVEYURL", $sugar_survey_Url);
    // End 
