        ...
        try {
            ...
            if ($statusProduct != 3) {
                ...
                $query = "SELECT
                  	...
                  	solicitudes.date_entered,solicitudes.date_modified, solicitudes.daypas, solicitudes.tct_etapa_ddw_c, solicitudes.tct_estapa_subetapa_txf_c as oppEtapa,
                  	cuentas.status_management_c as EstatusProducto, cuentas.tipo_producto, solicitudes.tipo_producto_c,
                  	CASE WHEN solicitudes.date_entered < DATE_SUB(now(), INTERVAL 5 DAY) THEN 0
                  	WHEN solicitudes.date_entered > DATE_SUB(now(), INTERVAL 5 DAY) THEN 1
                  	END AS semaforo
                  FROM
                      ...
                      (SELECT
                  		...
                  		oppcstm.tct_estapa_subetapa_txf_c, opp.date_entered,
                  		opp.amount as monto
