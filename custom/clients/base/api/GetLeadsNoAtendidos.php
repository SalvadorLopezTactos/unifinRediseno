    ...
    public function getcstmLeadNoAtendido($api, $args)
    {
        ...
        try {
            ...
            if ($estatusProduct != 3) {
                ...
                $query = "SELECT idLead, nombre, subtipo, estatus, max(semaforo) as semaforo
                FROM (
                    SELECT * FROM
                     (	SELECT le.idLead, le.assigned_user_id, le.nombre, le.subtipo, le.estatus,
                      ...
                      SELECT -- count(*) 
                        DISTINCT(l.id) as idLead, l.assigned_user_id, lc.name_c as nombre, lc.subtipo_registro_c as subtipo, lc.status_management_c as estatus,
                        ...
                        WHERE   l.assigned_user_id = '{$id_user}' AND 
                        lc.subtipo_registro_c in (1,2)
                        AND (lc.status_management_c = '{$estatusProduct}'  or lc.status_management_c is null)
                        AND (lc.contacto_asociado_c = 0 or lc.contacto_asociado_c is null)
                UNION
                    SELECT -- count(*) 
