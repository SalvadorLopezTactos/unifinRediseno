...
ac.tipo_registro_cuenta_c as tipoCuenta, ac.subtipo_registro_cuenta_c as subtipoCuenta,
 opp.id as idOpp, opp.name as oppNombre, opp.id, opp.amount as monto,
 opp.date_entered, opp.assigned_user_id, oppcstm.tct_etapa_ddw_c,
 oppcstm.tct_estapa_subetapa_txf_c as oppEtapa, up.name , upc.fecha_asignacion_c ,
 up.tipo_producto, upc.status_management_c as EstatusProducto
FROM accounts a 
...
and ((upc.fecha_asignacion_c > DATE_SUB(now(), INTERVAL 10 DAY) and opp.amount <= 10000000) OR
(upc.fecha_asignacion_c > DATE_SUB(now(), INTERVAL 20 DAY) and opp.amount > 10000000))";
               //$GLOBALS['log']->fatal('query',$query);
                ...
                while ($row = $GLOBALS['db']->fetchByAssoc($result)){                    
                    $records_in['records'][] = array(
						...
						'oppEtapa' => $row['oppEtapa'], 'EstatusProducto' => $row['EstatusProducto'],
