    ...
    loadEnumOptions: function(fetch, callback) {
        ...
        if (fetch || !this.items) {
            ...
            if (this.context.get(_key)) {
                ...
                var activo_subactivo_url = app.api.buildURL('subActivoAPI?activo=' + this.model.get('sub_activo_c'),
                    null, null, null);
    ...
    bindDataChange: function() {
        if (this.model) {
            this.model.on('change:' + this.name, function() {
                /** BEGIN CUSTOMIZATION: jescamilla@levementum.com 7/21/2015 Description: Reload Sub Activo*/
                var field = this.view.getField('sub_activo_3_c');
                field.loadEnumOptions(false, function() {
                    console.log('Sub Activo 2 Refrescado');
                });
                /* END CUSTOMIZATION */
