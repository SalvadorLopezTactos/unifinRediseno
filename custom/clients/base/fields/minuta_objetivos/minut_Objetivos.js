...
({
    ...
    btnaddObjetivo: function (evt) {
        if (!evt) return;
        if ($('.newObjetivo').val()=="" || $('.newObjetivo').val()== null ) {

            $(evt.currentTarget.children[0]).css('color','red');

        }else {
            $(evt.currentTarget.children[0]).css('color','green');
