({
  ...
  muestraChecklist: function (e) {
    ...
    if (this.tipo_registro != undefined) {
      ...
      app.api.call(
        "GET",
        app.api.buildURL(
          "GetChecklistKanban/" + this.id_registro + "/" + this.tipo_registro
        ),
        null,
        {
          success: function (data) {
            ...
            if (
              contextoKanban.subtipo == 1 ||
              contextoKanban.subtipo == 2 ||
              contextoKanban.subtipo == 7
            ) {
              ...
              $(".center-title-checklist").append(
                '<div><span class="close closeChecklist" tabindex="-1" style="margin:5px;color:black">X</span></div>'
              );
              $(".center-title-checklist").append(
                '<p style="margin-top: 40px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"><a href="#' +
                  contextoKanban.data_link +
                  '" target="_blank">' +
                  contextoKanban.nombre_link +
                  "</a><br><b>Checklist Actividades Pendientes</b></p>"
              );
  ...
  setFavorite: function (e) {
    ...
    if (estilo == undefined) {
      ...
      app.api.call(
        "update",
        app.api.buildURL(modulo + "/" + id + "/favorite"),
        null,
        {
          success: function (data) {
            ...
            var indice = contextoKanban.searchIndexFavorite(
              contextoKanban.idRegistroGlobal,
              contextoKanban.registrosKanban[contextoKanban.columna].Registros
            );
            ...
            for (
              ...
              if (
                contextoKanban.registrosKanban[contextoKanban.columna]
                  .Registros[index].Favorito != null
              ) {
                favoritos.push(
                  contextoKanban.registrosKanban[contextoKanban.columna]
                    .Registros[index]
      ...
      app.api.call(
        "update",
        app.api.buildURL(modulo + "/" + id + "/unfavorite"),
        null,
        {
          success: function (data) {
    ...
    if (this.registrosKanban[columna].Registros.length > 0) {
      if (modoOrdenamiento == "DESC") {
        ...
        this.registrosKanban[columna].Registros.sort((a, b) => {
          return (
            parseFloat(a.Monto_Cuenta.replace(/,/g, "")) -
            parseFloat(b.Monto_Cuenta.replace(/,/g, ""))
          );
        });
      } else {
      ...
      $('[data-columna="' + columna + '"].sortTipo').attr(
        "modo-ordenamiento",
        valorOrdenamiento
      );
    }
  ...
  ordenamientoPorTipo: function (e) {
    ...
    var columna = $(e.currentTarget).attr("data-columna");
    var registros = this.ordenaRegistros(
      contextoKanban.registrosKanban,
      columna,
      tipo_ordenamiento
    );
  ...
  ordenaRegistros: function (arreglo, columna, tipo) {
    ...
    switch (tipo) {
      case "MA":
        ...
        for (
          let index = 0;
          index < arreglo[columna].Registros.length;
          index++
        ) {
