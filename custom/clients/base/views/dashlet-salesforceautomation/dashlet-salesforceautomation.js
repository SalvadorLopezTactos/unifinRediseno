({
    ...
    loadData: function (options) {
        ...
        app.api.call("read", this.reporteesEndpoint, {}, {
            success: _.bind(function (data) {
                ...
                app.api.call("create", app.api.buildURL("SalesforceAutomation", '', {}, {}), {data: sfa_options}, {
                    success: _.bind(function (data) {
                        if (self.disposed) {
                            return;
                        }
                        _.extend(self, {c1_rows: data});
                        self.render();

                        var clonedHeaderRow;

                        $(".c1_box").each(function() {

                            var columnSizes = {}
                            $(".c1_box").find('th').each(function(i, el){
                                columnSizes[i] = $(el).width();
                            });

                            clonedHeaderRow = $(".c1_header", this);

                            clonedHeaderRow
                                .before(clonedHeaderRow.clone())
                                .css("width", clonedHeaderRow.width())
                                .addClass("c1HeaderFloat")
                        ...
                        $('.dashlet-content > div').each(function(){
                            if($(this).children('div').attr('class') == 'dashlet-estatus-del-proceso-wrapper'){
                        ...
                        function toggleHeader() {
                            ...
                            var scrollTop = $('.dashlet-estatus-del-proceso-wrapper').parent().offset().top;

                            $(".c1_box").each(function() {

                                var el             = $(this),
                                    offset         = el.offset(),
                                    c1HeaderFloat = $(".c1HeaderFloat", this)

                                console.log ('(C1) scrollTop: ' + scrollTop + ' - offset.top: ' + offset.top + ' -  el.height(): ' +  el.height());

                                var trackingOffset = offset.top + 115;
                                if ((scrollTop > trackingOffset) && (scrollTop < trackingOffset + el.height())) {
                                    $('.c1HeaderFloat').css('top', scrollTop + 'px');
                                    c1HeaderFloat.css({
                                        "visibility": "visible"
                                    });
                                } else {
    ...
    obtenOperaciones: function (sfa_options) {
        var obtenOperacionesUrl = app.api.buildURL("SalesforceAutomation", '', {}, {});
    ...
    moverForecast: function (e) {
        ...
        var obtenOperacionesUrl = app.api.buildURL("SalesforceAutomationActions", '', {}, {});
        app.api.call("create", obtenOperacionesUrl, {data: Params}, {
            success: _.bind(function (data) {
                if (self.disposed) {
                    return;
