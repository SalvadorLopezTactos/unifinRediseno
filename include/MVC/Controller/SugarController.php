...
class SugarController
{
    ...
    private function blockFileAccess()
    {
        ...
        if (!empty($GLOBALS['sugar_config']['admin_access_control']) && $GLOBALS['sugar_config']['admin_access_control']) {
            ...
            if (!empty($this->file_access_control_map['modules'][$module]['actions']) && (safeInArray($action, $this->file_access_control_map['modules'][$module]['actions']) || !empty($this->file_access_control_map['modules'][$module]['actions'][$action]))) {
                //check params
                if (!empty($this->file_access_control_map['modules'][$module]['actions'][$action]['params'])) {
                    ...
                    foreach ($params as $param => $paramVals) {
                        if (!empty($_REQUEST[$param])) {
                            if (!safeInArray($_REQUEST[$param], $paramVals)) {
                                $block = false;
                                break;
                            }
                        }
