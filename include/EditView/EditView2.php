...
class EditView
{
    ...
    public function render()
    {
        ...
        $this->sectionPanels = [];
        $this->sectionLabels = [];
        if (!empty($this->defs['panels']) && safeCount($this->defs['panels']) > 0) {
        ...
        foreach (safeIsIterable($this->defs['panels']) ? $this->defs['panels'] : [] as $key => $p) {
            ...
            if (!is_array($this->defs['panels'][$key])) {
                ...
                foreach ($p as $row => $rowDef) {
                    $columnsInRows = safeCount($rowDef);
                    $columnsUsed = 0;
                    foreach ($rowDef as $col => $colDef) {
    ...
    protected function getPanelWithFillers($panel)
    {
        $addFiller = true;
        foreach ($panel as $row) {
            if (safeCount($row) == $this->defs['templateMeta']['maxColumns']
                || 1 == safeCount($panel)) {
                $addFiller = false;
                break;
            }
        ...
        if ($addFiller) {
            $rowCount = safeCount($panel);
            $filler = safeCount($panel[$rowCount - 1]);
        ...
        if ($this->focus) {
            ...
            foreach ($this->focus->toArray() as $name => $value) {
                ...
                if (isset($this->fieldDefs[$name]['options']) && is_array($this->fieldDefs[$name]['options']) && isset($this->fieldDefs[$name]['default_empty']) && !isset($this->fieldDefs[$name]['options'][$this->fieldDefs[$name]['default_empty']])) {
                    $this->fieldDefs[$name]['options'] = array_merge([$this->fieldDefs[$name]['default_empty'] => $this->fieldDefs[$name]['default_empty']], $this->fieldDefs[$name]['options']);
                }
                ...
                if (isset($this->fieldDefs[$name]['function'])) {
                    ...
                    $functionArgs = [$this->focus, $name, $value, $this->view];
                    ...
                    if ($function == 'getCurrencies' && $functionBean == 'Currencies') {
                        $function = ['returns' => 'html', 'name' => 'getCurrencyDropDown'];
                        $functionBean = [];
            ...
            $this->focus->ACLFilterFieldList($this->fieldDefs, [], ['add_acl' => true]);
