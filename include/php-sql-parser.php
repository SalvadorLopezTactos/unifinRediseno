...
class PHPSQLParser
{
    ...
    private function count_paren($token, $chars = ['(', ')'])
    {
        ...
        return ['open' => $open, 'close' => $close, 'balanced' => (safeCount($close) - safeCount($open))];
    }
...
EOREGEX;
            ...
            if ($token && $token[0] == '(') {
                ...
                while ($needed > 0 && $n < $token_count - 1) {
                    ...
                    if ($closes != $needed) {
                        ...
                        $pos = $info2['close'][safeCount($info2['close']) - 1];
                        $str1 = $str2 = '';
                        if ($pos == 0) {
                            $str1 = ')';
                        } else {
    ...
    */
    private function process_limit($tokens)
    {
        ...
        for ($i = $pos; $i < safeCount($tokens); ++$i) {
            if ($tokens[$i] != '') {
                $end = $tokens[$i];
                break;
            }
    ...
    */
    private function process_select_expr($expression)
    {
        ...
        */
        ...
        if (!$alias && $last['expr_type'] == 'colref' && safeCount($stripped) > 0) {
    ...
    */
    ...
    private function load_reserved_words()
    {
        ...
        for ($i = 0; $i < safeCount($this->reserved); ++$i) {
