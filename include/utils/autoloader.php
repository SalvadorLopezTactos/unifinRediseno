...
{
    public const CLASS_CACHE_FILE = 'class_map.php';

    /**
     * Root namespace
     */
    public const NS_ROOT = 'Sugarcrm\\Sugarcrm\\';
     ...
     */
    public static $namespaceMapPsr4 = [];
    ...
    {
        spl_autoload_register(['SugarAutoLoader', 'autoload'], true, $prepend);
    }
    ...
    protected static function getFilenameForViewClass($class)
    {
        ...
        $viewClass = substr($class, 0, 4);
        if ($viewClass == 'View' && $view !== '') {
            if ($module) {
    ...
    public static function existing()
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_array($file)) {
                $out += call_user_func_array(['SugarAutoLoader', 'existing'], $file);
                continue;
            }
            if (file_exists($file)) {
                $out[] = $file;
    ...
    public static function existingCustom()
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_array($file)) {
                $out += call_user_func_array(['SugarAutoLoader', 'existingCustom'], $file);
                continue;
            }
            if (file_exists($file)) {
                $out[] = $file;
    ...
    {
        $files = func_get_args();
        $out = call_user_func_array(['SugarAutoLoader', 'existingCustom'], $files);
        if (empty($out)) {
            return null;
        } else {
    ...
    protected static function getFilenameForExpressionClass($class)
    {
        if (substr($class, -10) == 'Expression') {
            ...
            $types = ['Boolean', 'Date', 'Enum', 'Generic', 'Numeric', 'Relationship', 'String', 'Time'];
