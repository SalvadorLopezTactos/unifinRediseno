...
class SugarBean
{
    ...
    */
    ...
    public function getActivityEnabledFieldDefinitions($excludeType = ['datetime'])
    {
        if (!isset($this->activity_enabled_fields)) {
            ...
            foreach ($this->field_defs as $field => $properties) {
                ...
                if ($field != 'modified_user_id' && !empty($field_type) && !safeInArray($field_type, $excludeType)) {
    ...
    public function getFieldDefinitions(?string $property = null, array $filter = []): array
    {
        ...
        return array_filter($definitions, function (array $def) use ($property, $filter): bool {
            return isset($def[$property]) && safeInArray($def[$property], $filter);
        });
    }
    ...
    public function unPopulateDefaultValues()
    {
        ...
        foreach ($this->field_defs as $field => $value) {
            ...
            if (!empty($this->$field) && !empty($value['display_default']) && safeInArray($value['type'], ['date', 'datetime', 'datetimecombo']) &&
    ...
    public function cleanBean()
    {
        foreach ($this->field_defs as $key => $def) {
            ...
            if (safeInArray($def['type'], $this->getHtmlFieldTypes())) {
    ...
    protected function getTemplateNameForNotificationEmail()
    {
        ...
        if (!safeInArray('set_notification_body', get_class_methods($this))) {
    ...
    public function send_assignment_notifications($notify_user, $admin)
    {
        if (($this->object_name == 'Meeting' || $this->object_name == 'Call')
            ...
            if (isset($this->current_notify_user->id)) {
                if (safeInArray($subject, $this->subjectsLog[$this->current_notify_user->id] ?? [], true)) {
                    return;
                }
    ...
    protected function set_relationship_info($exclude = [])
    {
        ...
        if ($rel_id && $rel_link && !safeInArray($rel_link, $exclude) && $rel_id != $this->id) {
            $new_rel_id = $rel_id;
    ...
    protected function handle_remaining_relate_fields($exclude = [])
    {
        ...
        foreach ($this->field_defs as $def) {
            if ($def ['type'] == 'relate' && isset($def ['id_name']) && isset($def ['link'])) {
                $linkField = $def ['link'];
                if (isset($def['save'])) {
                    if (safeInArray($def['id_name'], $exclude) || safeInArray($def['id_name'], $this->relationship_fields)) {
    ...
    protected function update_parent_relationships($exclude = [])
    {
        foreach ($this->field_defs as $def) {
            if (!empty($def['type']) && $def['type'] == 'parent') {
                ...
                if (safeInArray($idField, $exclude)) {
                    continue;
                }
            ...
            if ($this->load_relationship($new_rel_link)) {
                ...
                if ($this->load_relationship($lower_link)) {
                    ...
                    if (BeanFactory::getBeanClass($new_rel_id)) {
                        ...
                        if (!empty($rels)) {
                            foreach ($this->field_defs as $field => $def) {
                                if ($def['type'] == 'link' && !empty($def['relationship']) && safeInArray($def['relationship'], $rels)) {
            ...
            switch ($def['type']) {
                ...
                case 'bool':
                    if (empty($this->$field)) {
                        ...
                        $this->$field = true;
                    } elseif (safeInArray(strval($this->$field), $boolean_false_values)) {
                        $this->$field = false;
                        $reformatted = true;
                    } else {
                        $this->$field = true;
        ...
        if (empty($final_query)) {
            ...
            foreach ($subqueries as $i => $subquery) {
                ...
                foreach ($query_fields as $field => $select) {
                    if (!safeInArray($field, $all_fields)) {
                        $all_fields[] = $field;
                    }
                }
            ...
            if ($this->is_relate_field($field)) {
                ...
                if (!empty($this->{$data['link']})) {
                    ...
                    if (isset($data['db_concat_fields'])) {
                        $buildWhere = false;
                        if (safeInArray('first_name', $data['db_concat_fields']) && safeInArray('last_name', $data['db_concat_fields'])) {
                            $exp = '/\(\s*?' . $data['name'] . '.*?\%\'\s*?\)/';
    ...
    public function fill_in_relationship_fields()
    {
        ...
        foreach ($this->field_defs as $field) {
            ...
            if (safeInArray($field['type'], static::$relateFieldTypes)) {
    ...
    */
    ...
    public function convertField($fieldvalue, $fieldDef)
    {
        if (!empty($fieldvalue)) {
            if (!(isset($fieldDef['source']) &&
                    !safeInArray($fieldDef['source'], ['db', 'custom_fields', 'relate'])
    ...
    public function shouldSendAssignmentPushNotification($stateChanges)
    {
        ...
        if (safeInArray($this->module_name, $moduleDenyList)) {
            return false;
        }
    ...
    public function getRelateFieldQuery($field_def, $joinTableAlias, $selectedFields = [])
    {
        ...
        foreach ($fields as $alias => $field) {
            if (!safeInArray($alias, $selectedFields)) {
                $parts[] = $field . ' ' . $alias;
            }
        }
    ...
    /**
     * Checks to see if this instance is licensed for ENTERPRISE
     * @return boolean
     */
    final public function isLicensedForEnterprise(): bool
    {
        $subs = $this->getSubsctiptionKeys();
        return array_key_exists(Subscription::SUGAR_BASIC_KEY, $subs);
    }

    /**
     * Return keys of available subscriptions
     * @return array
     */
    protected function getSubsctiptionKeys(): array
    {
        return SubscriptionManager::instance()->getAllSystemSubscriptionKeys();
    }
