...
(function(app) {
    ...
    function _fetchLabels(metadata, options) {
        ...
        app.lang.defaultLanguage = labels['default'];
        ...
        if (options.language && labels[options.language]) {
            ...
            currentLanguage = app.lang.defaultLanguage;
        ...
        app.lang.currentLanguage = currentLanguage;
        ...
        app.api.call("read", langStringsUrl, null, {
            ...
            error: function(err) {
                //Compatibility wrapper from when labels were loaded with jquery directly rather than with SugarAPI
                if (err.responseText == "invalid json") {
                    options.error({
                        code: "sync_failed",
                        label: "ERR_SYNC_FAILED"
                    });
    ...
    app.augment("metadata", {
        ...
        getModuleNames: function(options) {
            ...
            moduleList = _.keys(_metadata.modules_info);

            var filter;
            if (options.filter && _.indexOf(filters, options.filter) > -1) {
                filter = options.filter;
            } else if (options.filter) {
                app.logger.warn('Can\'t filter getModuleNames by ' + options.filter);
            }

            if (filter) {
                var opposite = filter.charAt(0) === '!';
                if (opposite) {
                    moduleList = _.keys(_metadata.modules_info);
                    ...
                    // Filter the list
                    moduleList = _.filter(moduleList, function(module) {
                        return _metadata.modules_info[module][filter];
                    });
                }
            }

            if (!_.isEmpty(app.user.get('module_list'))) {
                if (filter === 'display_tab') {
