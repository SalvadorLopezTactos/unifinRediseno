...
({
     ...
     * @override
     *
     * Opens config layout, calling `toggle` if we are in CCP only mode so the
     * full console is visible. Toggle is called before sliding drawer up
     * so the user only sees the full drawer appear from the bottom.
     */
    open: function() {
        if (!this.isOpen()) {
            this._setSize();
     ...
     * @override
     *
     * Do nothing as the config view has dummy data
     */
