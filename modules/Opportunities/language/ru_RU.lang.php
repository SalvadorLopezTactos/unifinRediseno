...
// BEGIN ENT/ULT
    ...
    // List View Help Text
    'LBL_HELP_RECORDS' => 'Модуль {{plural_module_name}} позволяет отслеживать отдельные продажи от начала и до конца. Каждая запись модуля {{module_name}} является потенциальной сделкой и содержит соответствующие данные по продаже, а также относится к другим важным записям, таким как {{quotes_module}}, {{contacts_module}} и т. д. Каждый модуль {{module_name}} будет проходить несколько стадий продаж до тех пор, пока не получит статус «Успешно закрыто» или «Потеряно». Эффективность модуля {{plural_module_name}} можно увеличить, используя модуль {{forecasts_singular_module}} системы Sugar для понимания и прогнозирования тенденций продаж и сосредоточения работы на достижении установленных квот.',
...
- Чтобы редактировать поля данной записи, нажмите нужное поле или кнопку «Редактировать». 
...
- Чтобы оставлять и просматривать пользовательские комментарии, а также журнал изменений записи в {{activitystream_singular_module}}, переключите левую нижнюю панель в режим «Лента активности». 
- Чтобы подписаться на данную запись или добавить ее в избранное, используйте значки справа от записи. 
...
Чтобы создать модуль {{module_name}}, выполните такие действия: 
