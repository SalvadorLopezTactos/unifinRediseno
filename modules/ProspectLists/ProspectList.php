...
class ProspectList extends SugarBean
{
    ...
    {
        global $current_user;
        $currentUserId = $this->db->quoted($current_user->id);
        return sprintf(
            ...
            WHERE (
                ( l.team_set_id IN (select tst.team_set_id from team_sets_teams tst
                    INNER JOIN team_memberships team_membershipsleads ON tst.team_id = team_membershipsleads.team_id
                    AND team_membershipsleads.user_id = {$currentUserId}
                    AND team_membershipsleads.deleted = 0) ) AND
                plp.prospect_list_id = %1\$s AND plp.related_type = 'Leads' AND plp.deleted=0
            AND l.deleted=0
        ...
        UNION ALL
            ...
            LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
            WHERE (
                ( u.team_set_id IN (select tst.team_set_id from team_sets_teams tst
                    INNER JOIN team_memberships team_membershipsusers ON tst.team_id = team_membershipsusers.team_id
                    AND team_membershipsusers.user_id = {$currentUserId}
                    AND team_membershipsusers.deleted = 0) ) AND
                plp.prospect_list_id = %1\$s AND plp.related_type = 'Users' AND plp.deleted=0
            AND u.deleted=0
            AND (ear.deleted=0 OR ear.deleted IS NULL))
        UNION ALL 
            ...
            LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
            WHERE (
                ( c.team_set_id IN (select tst.team_set_id from team_sets_teams tst
                    INNER JOIN team_memberships team_membershipscontacts ON tst.team_id = team_membershipscontacts.team_id
                    AND team_membershipscontacts.user_id = {$currentUserId}
                    AND team_membershipscontacts.deleted = 0) ) AND
                plp.prospect_list_id = %1\$s AND plp.related_type = 'Contacts' AND plp.deleted=0
            AND c.deleted=0
            AND (ear.deleted=0 OR ear.deleted IS NULL))
        UNION ALL 
            ...
            LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
            WHERE (
                ( p.team_set_id IN (select tst.team_set_id from team_sets_teams tst
                    INNER JOIN team_memberships team_membershipsprospects ON tst.team_id = team_membershipsprospects.team_id
                    AND team_membershipsprospects.user_id = {$currentUserId}
                    AND team_membershipsprospects.deleted = 0) ) AND
                plp.prospect_list_id = %1\$s AND plp.related_type = 'Prospects' AND plp.deleted=0
            AND p.deleted=0
            AND (ear.deleted=0 OR ear.deleted IS NULL))
        UNION ALL 
            ...
            LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
            WHERE (
                ( a.team_set_id IN (select tst.team_set_id from team_sets_teams tst
                    INNER JOIN team_memberships team_membershipsaccounts ON tst.team_id = team_membershipsaccounts.team_id
                    AND team_membershipsaccounts.user_id = {$currentUserId}
                    AND team_membershipsaccounts.deleted = 0) ) AND
                plp.prospect_list_id = %1\$s AND plp.related_type = 'Accounts' AND plp.deleted=0
            AND a.deleted=0
            AND (ear.deleted=0 OR ear.deleted IS NULL))
