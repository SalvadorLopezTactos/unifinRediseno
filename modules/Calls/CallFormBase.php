...
{
    protected $repeatDataArray = [];

    protected $recurringCreated = [];
...
EOQ;
    ...
    protected function processRecurring(Call $focus)
    {
        if (!empty($_REQUEST['edit_all_recurrences'])) {
            ...
            $focus->markRepeatDeleted();
        }
        if (safeCount($this->repeatDataArray) > 0) {
            // prevent sending invites for recurring activities
            unset($_REQUEST['send_invites'], $_POST['send_invites']);
            $this->recurringCreated = $focus->createRecurringEvents($this->repeatDataArray);
        }
    }
    ...
    public function prepareRecurring()
    {
        ...
        if (empty($_REQUEST['edit_all_recurrences'])) {
            $repeatFields = ['type', 'interval', 'count', 'until', 'dow', 'parent_id'];
            ...
            $this->repeatDataArray = CalendarEventsUtils::getInstance()->buildRecurringSequence(
                $_REQUEST['date_start'],
                $params
            );
    ...
    public function checkRecurringLimitExceeded()
    {
        ...
        if (safeCount($this->repeatDataArray) > ($limit - 1)) {
            return $limit;
        }
        return false;
    }
