...
function compare($table_name, $db_indexes, $var_indexes)
{
    ...
    foreach ($var_indexes as $var_i_name => $var_i_def) {
        ...
        $sel_db_index = null;
        $var_fields_string = '';
        if (safeCount($var_i_def['fields']) > 0) {
...
(function_exists('logThis')) ? logThis('found ' . safeCount($beanFiles) . ' Beans to process') : '';
(function_exists('logThis')) ? logThis('found ' . safeCount($dictionary) . ' Dictionary entries to process') : '';
...
(function_exists('logThis')) ? logThis('RepairIndex: we have ' . safeCount($drop_index) . ' indices to DROP.') : '';
(function_exists('logThis')) ? logThis('RepairIndex: we have ' . safeCount($add_index) . ' indices to ADD.') : '';
(function_exists('logThis')) ? logThis('RepairIndex: we have ' . safeCount($change_index) . ' indices to CHANGE.') : '';

if ((safeCount($drop_index) > 0 or safeCount($add_index) > 0 or safeCount($change_index) > 0)) {
