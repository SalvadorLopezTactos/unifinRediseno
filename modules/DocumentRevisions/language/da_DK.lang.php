<?php
/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/Resources/Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */
/*********************************************************************************
 * Description:  Defines the English language pack for the base application.
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
 ********************************************************************************/

$mod_strings = [
    //module
    'LBL_MODULE_NAME' => 'Dokumentrevision',
    'LBL_MODULE_NAME_SINGULAR' => 'Dokumentrevision',

    'LNK_NEW_DOCUMENT' => 'Opret dokument',
    'LNK_DOCUMENT_LIST' => 'Dokumentliste',

    'LBL_REVISIONS_FOR_DOC' => 'Revisioner til relateret dokument',

    'LBL_RECORD_SAVED_SUCCESS' => 'Du har med succes oprettet {{moduleSingularLower}} <a href="#{{buildRoute model=this}}"> {{revision}} </a>.', // use when a model is available


    //vardef labels
    'LBL_REVISION_NAME' => 'Revisionsnummer',
    'LBL_FILENAME' => 'Filnavn',
    'LBL_MIME' => 'Mime-type',
    'LBL_REVISION' => 'Revision',
    'LBL_DOCUMENT' => 'Relateret dokument',
    'LBL_LATEST_REVISION' => 'Seneste revision',
    'LBL_CHANGE_LOG' => 'Ændringslog',
    'LBL_ACTIVE_DATE' => 'Udgivelsesdato',
    'LBL_EXPIRATION_DATE' => 'Udløbsdato',
    'LBL_FILE_EXTENSION' => 'Filtype',
    'LBL_DET_CREATED_BY' => 'Oprettet af:',
    'LBL_DET_DATE_CREATED' => 'Oprettet den:',

    'LBL_DOC_NAME' => 'Dokumentnavn:',
    'LBL_DOC_VERSION' => 'Revision:',

    //document revisions.
    'LBL_REV_LIST_REVISION' => 'Revision',
    'LBL_REV_LIST_ENTERED' => 'Oprettet den',
    'LBL_REV_LIST_CREATED' => 'Oprettet af',
    'LBL_REV_LIST_LOG' => 'Ændringslog',
    'LBL_REV_LIST_FILENAME' => 'Filnavn',

    'LBL_CURRENT_DOC_VERSION' => 'Seneste Revision:',
    'LBL_SEARCH_FORM_TITLE' => 'Søg efter dokument',

    //error messages
    'ERR_FILENAME' => 'Filnavn',
    'ERR_DOC_VERSION' => 'Dokumentversion',
    'ERR_DELETE_CONFIRM' => 'Vil du slette denne dokumentrevision?',
    'ERR_DELETE_LATEST_VERSION' => 'Du har ikke tilladelse til at slette den seneste revision af et dokument.',
    'LNK_NEW_MAIL_MERGE' => 'Brevfletning',
    'LBL_DOC_ID' => 'Dokument kilde id',
    'LBL_DOC_TYPE' => 'Kilde',
    'LBL_DOC_URL' => 'Dokument kilde URL',
];
